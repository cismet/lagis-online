...
class DefaultHeader extends Component {
	render() {
		...
		for (let index = 0; index < gem.length; ++index) {
			gemarkungen.push({ value: gem[index].schluessel, label: gem[index].bezeichnung });
		}

		var flu = getFlure(flurstueck.fk_flurstueck_schluessel.fk_gemarkung.schluessel);
		const flure = [];
...
DefaultHeader.contextType = FlurstueckContext;
