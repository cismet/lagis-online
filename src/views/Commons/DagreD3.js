...
class DagreD3 extends React.Component {
	constructor(props) {
		super(props);
		if (this.props.dotGraph !== undefined) {
			this.graph = graphlibDot.read(this.props.dotGraph);
		}
	}
	...
	renderDag() {
		let g;
		if (this.graph !== undefined || this.props.graph !== undefined) {
			g = this.graph || this.props.graph;
		} else {
			g = new dagreD3.graphlib.Graph().setGraph({});

			for (let [ id, node ] of Object.entries(this.props.nodes)) g.setNode(id, node);
		...
		if (this.props.onNodeClick)
			svg.selectAll('.dagre-d3 .node').on('click', (id) => this.props.onNodeClick(id));
	}
